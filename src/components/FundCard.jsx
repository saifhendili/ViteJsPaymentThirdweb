...
const FundCard = ({ pId,owner, title, description, target, deadline, amountCollected, image }) => {
  const remainingDays = daysLeft(deadline);
  const { PlaceOrder} = useStateContext();
  const [isLoadingPaymeny,setisLoadingPaymeny]=useState(false)
    const handleOrder = async () => {
      setisLoadingPaymeny(true);
  console.log('11111111111111111'+pId)
      await PlaceOrder(pId, target); 
      setisLoadingPaymeny(false);
      console.log('1111111111111111122222')

    }
  
  return isLoadingPaymeny ? <Loader />:
